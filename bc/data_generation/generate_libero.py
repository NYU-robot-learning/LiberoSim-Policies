...
DATASET_PATH = Path('/home/siddhant/github/LIBERO/libero/datasets')
BENCHMARKS = ['libero_10', 'libero_90']
SAVE_DATA_PATH = Path('../../expert_demos/libero_with_langemb')
img_size = (128, 128)
...
for benchmark in BENCHMARKS:
	...
	for task_file in benchmark_path.glob('*.hdf5'):
		...
		for demo in data.keys():
			...
			observation = {}
			observation['robot_states'] = np.array(demo_data['robot_states'], dtype=np.float32)
			...
			for i in range(len(demo_data['states'])):
				...
				proprio = env.env.get_robot_state_vector() # [gripper_qpos, eef_pos, eef_quat]
				# append
				pixels.append(img)
				pixels_ego.append(img_ego)
				proprio.append(proprio)
			...
			observation['proprio'] = np.array(proprio, dtype=np.float32)
		...
		# save data
		save_data_path = save_benchmark_path / (str(task_file).split('/')[-1][:-10] + '.pkl')
