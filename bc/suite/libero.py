...
class RGBArrayAsObservationWrapper(dm_env.Environment):
	...
	"""
	def __init__(self, env, width=84, height=84, max_episode_len=300, max_state_dim=100):
		self._env = env
		self._width = width
		self._height = height
		...
		self._action_spec = specs.BoundedArray(action_spec[0].shape,
										np.float32,
										action_spec[0],
										action_spec[1],
										'action')
		...
		# robot_state = np.concatenate([obs['robot0_joint_pos'], obs['robot0_gripper_qpos']])
		robot_state = self._env.env.get_robot_state_vector(obs) # [gripper_qpos, eef_pos, eef_quat]
		self._obs_spec = {}
		self._obs_spec['pixels'] = specs.BoundedArray(shape=dummy_obs.shape,
													  dtype=np.uint8,
													  minimum=0,
													  maximum=255,
													  name='pixels')
		# self._obs_spec['pixels_egocentric'] = specs.BoundedArray(shape=dummy_obs.shape,
		# 												   		 dtype=np.uint8,
		# 														 minimum=0,
		# 														 maximum=255,
		# 														 name='pixels_egocentric')
		self._obs_spec['proprioceptive'] = specs.BoundedArray(shape=robot_state.shape,
															  dtype=np.float32,
															  minimum=-np.inf,
															  maximum=np.inf,
															  name='proprioceptive')
		self._obs_spec['features'] = specs.BoundedArray(shape=(self._max_state_dim,),
													  dtype=np.float32,
													  minimum=-np.inf,
													  maximum=np.inf,
													  name='features')
	...
	def reset(self, **kwargs):
		...
		# observation['proprioceptive'] = np.concatenate([obs['robot0_joint_pos'], obs['robot0_gripper_qpos']])
		observation['proprioceptive'] = self._env.env.get_robot_state_vector(obs)
		# get state
...
class FrameStackWrapper(dm_env.Environment):
	def __init__(self, env, num_frames):
		...
		self._obs_spec['proprioceptive'] = self._env.observation_spec()['proprioceptive']
		self._obs_spec['pixels'] = specs.BoundedArray(shape=np.concatenate(
			[[pixels_shape[2] * num_frames], pixels_shape[:2]], axis=0),
											...
											maximum=255,
											name='pixels')
		self._obs_spec['pixels_egocentric'] = specs.BoundedArray(shape=np.concatenate(
			[[pixels_shape[2] * num_frames], pixels_shape[:2]], axis=0),
											dtype=np.uint8,
											minimum=0,
											maximum=255,
	...
	def _extract_pixels(self, time_step):
		...
		return pixels.transpose(2, 0, 1).copy(), pixels_egocentric.transpose(2, 0, 1).copy()
...
class ActionDTypeWrapper(dm_env.Environment):
	...
	def step(self, action):
		...
		# step_type = StepType.LAST if (self._env._step == self._env._max_episode_len or observation['goal_achieved']) else StepType.MID
		step_type = StepType.LAST if (self._env._step == self._env._max_episode_len) else StepType.MID
		return TimeStep(
					step_type=step_type,
					reward=reward,
...
def make(suites, task_names, frame_stack, action_repeat, seed, height, width, max_episode_len, max_state_dim):
	envs = []
	task_descriptions = []
	...
	for suite_name in suites:
		...
		for task_name in task_names:
			...
			if task_name in task_suite.get_task_names():
				...
				# apply wrappers
				env = RGBArrayAsObservationWrapper(env, height=height, width=width, max_episode_len=max_episode_len, max_state_dim=max_state_dim)
