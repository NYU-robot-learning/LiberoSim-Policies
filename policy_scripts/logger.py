...
BC_TRAIN_FORMAT = [('step', 'S', 'int'),
					   ('actor_loss', 'L', 'float'),
					   ('total_time', 'T', 'time')]
BC_EVAL_FORMAT = [('frame', 'F', 'int'), ('step', 'S', 'int'),
					  ('episode', 'E', 'int'), ('episode_length', 'L', 'int'),
					  ('episode_reward', 'R', 'float'),
					  ('imitation_reward', 'R_i', 'float'),
					  ('total_time', 'T', 'time')]
RL_TRAIN_FORMAT = [('frame', 'F', 'int'), ('step', 'S', 'int'),
					   ('episode', 'E', 'int'), ('episode_length', 'L', 'int'),
					   ('episode_reward', 'R', 'float'),
					   ('imitation_reward', 'R_i', 'float'),
					   ('buffer_size', 'BS', 'int'), ('fps', 'FPS', 'float'),
					   ('total_time', 'T', 'time')]
RL_EVAL_FORMAT = [('frame', 'F', 'int'), ('step', 'S', 'int'),
					  ('episode', 'E', 'int'), ('episode_length', 'L', 'int'),
					  ('episode_reward', 'R', 'float'),
					  ('imitation_reward', 'R_i', 'float'),
					  ('total_time', 'T', 'time')]
	...
	def __init__(self, log_dir, use_tb, mode='bc'):
		...
		if mode == 'bc':
			self._train_mg = MetersGroup(log_dir / 'train.csv',
										 formating=BC_TRAIN_FORMAT)
			self._eval_mg = MetersGroup(log_dir / 'eval.csv',
							   			formating=BC_EVAL_FORMAT)
		elif mode == 'rl':
			self._train_mg = MetersGroup(log_dir / 'train.csv',
									formating=RL_TRAIN_FORMAT)
			self._eval_mg = MetersGroup(log_dir / 'eval.csv',
										formating=RL_EVAL_FORMAT)
		if use_tb:
