...
class RGBArrayAsObservationWrapper(dm_env.Environment):
	...
	"""
	def __init__(self, env, ml1, width=84, height=84, max_path_length=125, camera_name="corner", max_state_dim=0):
		self._env = env
		self.ml1 = ml1
		self._width = width
		self._height = height
		...
		self._obs_spec['pixels'] = specs.BoundedArray(shape=self.observation_space.shape,
													  ...
													  maximum=255,
													  name='observation')
		self._obs_spec['features'] = specs.Array(shape=(self.max_state_dim,),
													  dtype=np.float32,
													  name='observation')
...
class ExtendedTimeStepWrapper(dm_env.Environment):
	...
	# def _add_attr(self, time_step, **kwargs):
	# 	return ExtendedTimeStep(observation=time_step.observation,
	# 					  		step_type=time_step.step_type,
	# 							action=time_step.action,
	# 							reward=time_step.reward,
...
def make(names, frame_stack, action_repeat, seed, max_episode_len, max_state_dim):
	envs = []
	task_descriptions = []
	for name in names:
		...
		# add wrappers
		env = RGBArrayAsObservationWrapper(env, ml1, max_path_length=max_episode_len, camera_name=CAMERA[name], max_state_dim=max_state_dim)
		...
		envs.append(env)
		# task description
		task_descriptions.append(" ".join(name.split('-')[:-1]))
